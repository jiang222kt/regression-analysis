...
int main(const int argc, const char **argv)
{
    ...
    if((fp = fopen("../data-with-geo.csv", "r")) == NULL){
        fprintf(stderr, "error: can't open %s\n", "data.csv");
        exit(EXIT_FAILURE);
    }

    char name[64];
    float alt, temp, lgt, lat;
    int ret;
    int i = 0;
    char *tmp;

    while (fgets(buf, bufsize, fp) != NULL ){
        ret = sscanf(buf, "%[^,],%f,%f,%f,%f\n", name, &alt, &temp, &lgt, &lat);
        if (ret != 5) {
        ...
        sample[i].alt = alt;
        sample[i].temp = temp;
        sample[i].lgt = (lgt - 35)*1.5;  // all locations reside around (35.xxxx ,137.xxx) 
        sample[i].lat = (lat -137)*1.5;  // so we do some calculation to amplify the difference
        i++;
    }
    ...
    */
    ...
    for (int i = 0 ; i < num ; i++){
	    printf("%2d: %-15s, %0.2f\t, %.2f, %.2f, %.2f\n",i, sample[i].loc, sample[i].alt, sample[i].temp, sample[i].lgt, sample[i].lat);
    }
    printf(" ==End of Data==\n");
    ...
    printf(" (y: temp, x1: alt, x2: lgt, x3: alt)\n");
